...
/* ---- Product names per case - FIXED to get single product ---- */
product_agg AS (
  SELECT /*+ MATERIALIZE */
         UC_INTERVENTION_ID,
         PROIZVOD
  FROM (
    SELECT 
           cs.UC_INTERVENTION_ID,
           prod.NAME AS PROIZVOD,
           ROW_NUMBER() OVER (PARTITION BY cs.UC_INTERVENTION_ID 
                              ORDER BY prod.NAME) AS rn
    FROM case_services cs
    JOIN ODYSSEY.PCG_PRODUCT_SERVICE pps
      ON pps.PRODUCT_SERVICE_ID = cs.PRODUCT_SERVICE_ID
     AND pps.DELETED = 0
  ...
  WHERE rn = 1  -- Only take the first product per case
),
...
SELECT
  ...
  CASE
    WHEN me.MEMBERSHIP_ID IS NOT NULL
...
LEFT JOIN ODYSSEY.SLS_PACKAGE sp
  ON sp.PACKAGE_ID = me.PACKAGE_ID
 AND sp.DELETED = 0
LEFT JOIN ODYSSEY.SLS_PACKAGE sp2
  ON sp2.PACKAGE_ID = me.PACKAGE_COMPOSITE_ID
 AND sp2.DELETED = 0
...
/* Aggregated product names per case - GUARANTEED 1:1 relationship */
